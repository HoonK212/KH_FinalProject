...
<div @mouseleave="cartOpen = !cartOpen" :class="cartOpen ? 'translate-x-0 ease-out' : 'translate-x-full ease-in'" class="z-50 fixed right-0 top-0 max-w-xs w-full h-full px-6 py-4 transition duration-300 transform overflow-y-auto bg-white border-l-2 border-gray-300">
	...
	<!-- 결제하기 -->
	<a onclick="location.href='<%=request.getContextPath() %>/shopping/basket'" class="flex items-center justify-center mt-4 px-3 py-2 bg-blue-600 text-white text-sm uppercase font-medium rounded hover:bg-blue-500 focus:outline-none focus:bg-blue-500" style="cursor: pointer;">
	<span>주문하기</span>
